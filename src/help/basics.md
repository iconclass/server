...
[&para;](#path){: name="path" } The following example from division 7 Bible shows the **hierarchical path** principle: all subsequent descents in the hierarchy take place by extending the notation to the right with a letter and more digits:
...
The core of the Iconclass systems thus also includes quite a large number of historical people, like saints and persons from classical history. Not only have they found their place in Iconclass as named entities, but their life stories can be further subdivided systematically by means of so-called **[structural digits](#struct)**. Here are two examples of this combination:
[&para;](#francis){: name="francis" }The notation **11H(FRANCIS)59** contains the saint's name in brackets while the structural digit **59** indicates the scene of the stigmatization:

    {{% aimg 11hfrancis59 %}}

In [**98B(CAESAR)68**](</98B(CAESAR)68>) the digits **68** are used because they encode the death of someone from classical history.

     {{% aimg 98bcaesar68 %}}

#### Keys

[&para;](#keys){: name="keys" } **Keys** are flexible elements of the Iconclass system that allow you to add more detail to a basic concept. They are shown in the browser as a clickable list of concept definitions between brackets, below the hierarchical "children" of a concept. A set of keys is often also hierarchically subdivided.
...
[&para;](#cross){: name="cross" } Subjects can be related in many different ways, both thematically and visually, and Iconclass contains a wide variety of cross references linking them.
...
In actual practice cross references are a useful discovery tool. They do not convey the _conclusion_ that images are related, but provide the _suggestion_ that further research might be useful. Angelika Kauffmann's **La Penserosa** was tagged with [**52B1**](/52B1){:target="read"} in the [Virtuelles Kupferstichkabinett's](http://kk.haum-bs.de/?id=m-a-kauffmann-v3-3011){:target="read"} database, while the young woman in the print by Pieter van den Berge, tagged with [**11R4**](/11R4){:target="read"} in the [Rijksmuseum's database](http://hdl.handle.net/10934/RM0001.COLLECT.78627){:target="read"}, is contemplating life as an independent woman, a married woman or a nun. Juxtaposing the two does not suggest any historical relationship; it is merely a hint, automatically generated by the structure of the Iconclass schedules.

{{% aimg 52b1_11r4 %}}

#### Keywords

[&para;](#keywords){: name="keywords" } Text about keywords of the entry vocabulary
