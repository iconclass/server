...
def to_metabotnik(db_filename, project_name, json_filename, data):
    ...
    for obj in mapping["images"]:
        ...
        insert_xy.append(
            (
                count,
                obj["x"] / width,
                obj["x"] / width + obj["width"] / width,
                obj["y"] / height,
                obj["y"] / height + obj["height"] / height,
            )
        )
        count += 1
        if count > 999999:
    ...
    cursor = db.cursor()
    cursor.execute(
        "INSERT INTO projects VALUES (?, ?, ?)", (project_name, width, height)
    )
    cursor.execute(f"CREATE TABLE {project_name}_tags (tag, obj_id)")
    cursor.execute(
        f"CREATE TABLE {project_name}_objs (id INTEGER PRIMARY KEY AUTOINCREMENT, obj)"
    )
    cursor.execute(
        f"CREATE VIRTUAL TABLE {project_name}_index USING rtree(id, x1, x2, y1, y2)"
    )
    cursor.executemany(f"INSERT INTO {project_name}_objs VALUES (?, ?)", insert_objs)
    cursor.executemany(
        f"INSERT INTO {project_name}_index VALUES (?, ?, ?, ?, ?)", insert_xy
    )
